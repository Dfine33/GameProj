from utils.common import manhattan
...
class SimplePolicy(DecisionPolicy):
    def decide(self, unit, gamestate):
        ...
        for e in gamestate.units:
            if e.team != unit.team and manhattan(unit.pos(), e.pos()) <= unit.vision:
                visible.append(e)
        for b in [gamestate.base_a, gamestate.base_b]:
            if b.team != unit.team and manhattan(unit.pos(), b.pos()) <= unit.vision:
                visible.append(b)
        target = None
        min_d = 10**9
        for v in visible:
            d = manhattan(unit.pos(), v.pos())
            if d < min_d:
                min_d = d
                target = v
...
class TwoPhasePolicy(DecisionPolicy):
    ...
    def decide(self, unit, gamestate):
        ...
        if manhattan(unit.pos(), other_base.pos()) <= unit.vision:
        ...
        if not g or manhattan(unit.pos(), g) <= 0:
